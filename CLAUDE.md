...
**IT Crafters Installer** — a two-layer installer that sets up a complete Java development
environment for beginner students on Ubuntu 22.04/24.04 inside WSL2 on Windows 10/11.
Layer 1 is `setup.ps1` (Windows bootstrap, the student's only entry point); layer 2 is
...
(decisions below override it where they conflict).
...
- All user-visible output goes through `lib/ui.sh` helpers and MUST be Estonian; comments
  and log-file content are English. Technical output goes to `~/.itcrafters/install.log`
